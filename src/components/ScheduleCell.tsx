...
import { cn, textBody1, textTitle1, textTitle2 } from '@/lib/utils';
...
export function ScheduleCell({ trackId, cell }: ScheduleCellProps) {
  ...
  if (cell.kind === 'label') {
    return <td className={textTitle1(styles.label)}>{cell.title}</td>;
  }
    ...
    <td className={cn('py-5 px-4 min-h-20', styles.session)}>
      <div className={'flex items-center justify-between'}>
        <ul className="min-w-0 flex flex-col gap-1">
          <li className={textTitle2('text-white')}>{cell.title}</li>
          {cell.speaker && (
            <li className={textBody1(styles.tint)}>{cell.speaker}</li>
          )}
        </ul>
        {cell.avatarSrc ? (
          <img
